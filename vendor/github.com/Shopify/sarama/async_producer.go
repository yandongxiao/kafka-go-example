...
type asyncProducer struct {
	...
	// key 是 broker 指针，value 是对应的 broker producer
...
func newAsyncProducer(client Client) (AsyncProducer, error) {
	...
	// 在 newAsyncProducer 最后开启了两个 goroutine，一个为 dispatcher，一个为 retryHandler 。
...
func (p *asyncProducer) dispatcher() {
	// key 是 topic 名称， values 是 producer message channel
	// 这里的 handler 是一个 unbuffered channel
	handlers := make(map[string]chan<- *ProducerMessage)
	shuttingDown := false

	// 从 producer input channel 获取一条 message
...
// 创建 topic producer 对象
// newTopicProducer 的这种写法，内部创建一个 chan 返回到外层，然后通过在内部新开一个 goroutine 来处理该 chan 里的消息，这种写法在后面还会遇到好几次。
func (p *asyncProducer) newTopicProducer(topic string) chan<- *ProducerMessage {
	...
	// 每个 topic dispatcher 创建一个协程，负责分发消息
...
func (tp *topicProducer) dispatch() {
	for msg := range tp.input {
		if msg.retries == 0 {
			// topicProducer 会确保 msg.Partition 的值存在，
		...
		// 找到这条消息所在的分区对应的 channel，然后把消息丢进去，如果不存在则新建 chan
	...
	// 关闭 topic dispacther 时，需要关闭 partition dispatchers
...
func (tp *topicProducer) partitionMessage(msg *ProducerMessage) error {
	...
	err := tp.breaker.Run(func() (err error) {
		...
		if requiresConsistency {
			...
			// 即：key->partition 的映射是否需要一致,如果强制指定需要一致,那么就算这个分区不可用了也会把消息发给该分区以保证一致性,
			// 未指定则只会把消息投递给可用分区.当使用 Key-Ordering 策略的时候需要设置为 true 才能保证同一个 Key 被投递到同一个分区.
...
func (pp *partitionProducer) dispatch() {
	...
	// 找到这个主题和分区的 leader 所在的 broker, 这个调用应该是没有走网络通信的
	pp.leader, _ = pp.parent.client.Leader(pp.topic, pp.partition)
	if pp.leader != nil {
		// 根据之前的套路我们知道，真正的逻辑肯定在pp.parent.getBrokerProducer(pp.leader) 这个方法里面。
		// 存在创建 go routine, 处理 broker producer 的业务逻辑
...
func (bp *brokerProducer) run() {
	...
	for {
		...
		case <-bp.timer: // 3. 如果间隔时间到了，也会将消息发出去
			bp.timerFired = true
		case output <- bp.buffer: // 4. 将 buffer 里的数据发送到 局部变量 output chan 里
			// 如果第五步需要发送就会给 output 赋值，这样下一轮 select 的时候case output <- bp.buffer: 这个 case 就可能会执行到，就会把消息发给 output，实际上就是发送给了 bp.output.
		...
		// 如果时间到了，或者 buffer 里的消息达到阈值后都会触发真正的发送逻辑，这里实现比较有意思，需要发送的时候就把 bp.output
		// 也就是存放真正需要发送的批量消息的 chan 赋值给局部变量 output，如果不需要发送就把局部变量 output 清空
