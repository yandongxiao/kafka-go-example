...
// req.AddMessage(topic, partition, compMsg) 的最终作用就是给 message 补充对应的 topic 和 partition
func (ps *produceSet) buildRequest() *ProduceRequest {
	...
	for topic, partitionSets := range ps.msgs {
		for partition, set := range partitionSets {
			...
			// 处理压缩相关的逻辑, 默认是 CompressionNone
			if ps.parent.conf.Producer.Compression == CompressionNone {
				// 批量发送消息到 kafka server, 以 message set 为单位
				req.AddSet(topic, partition, set.recordsToSend.MsgSet)
			} else {
				// 下面是对 else 分支的整体描述
				// When compression is enabled, the entire set(待发送的消息们) for each partition is compressed
				// and sent as the payload of a single fake "message" with the appropriate codec
				// set and no key(不设置key, 设置编码集). When the server sees a message with a compression codec, it
				// decompresses(broker解压，在message set进行校验) the payload and treats the result as its message set.
				...
				// 按照 消息集合 为单位进行压缩的,
				// TODO: 竟然有 MetricRegistry, 可以在客户端收集 sarama 相关的数据！！！！
				...
				compMsg := &Message{
					Codec:            ps.parent.conf.Producer.Compression,      // 指定压缩算法
					CompressionLevel: ps.parent.conf.Producer.CompressionLevel, // 指定压缩级别
					Key:              nil,                                      // key 为空
					Value:            payload,                                  // 压缩后的消息内容
