...
// 针对每个 topic 都会有一个对应的 partitioner
func NewMyPartitioner(topic string) sarama.Partitioner {
	return &myPartitioner{}
}

// Partition 返回的是分区的位置或者索引，并不是具体的分区号。比如有十个分区[0,1，2,3...9] 这里返回 0 表示取数组中的第0个位置的分区。
// Partition 是否需要是并发安全的？topic producer 负责给 message 赋予 partition，每个 topic producer 有自己的协程！
...
// RequiresConsistency key->partition 的映射是否需要一致,如果强制指定需要一致,那么就算这个分区不可用了也会把消息发给该分区以保证一致性，
